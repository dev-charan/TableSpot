import { useState, useEffect, useRef } from 'react';
import { useOutletContext } from 'react-router-dom';
import { useMutation, useQueryClient } from '@tanstack/react-query';
import { Save, Upload, X, MapPin, Phone, Globe, Clock, Image, Info, Star } from 'lucide-react';
import toast from 'react-hot-toast';
import api from '../../../api/axios';
import MapPicker from '../../../components/MapPicker';

const ALL_AMENITIES = ['WiFi', 'Parking', 'Pool', 'Gym', 'Restaurant', 'Spa', 'Bar', 'Room Service', 'Laundry', 'Airport Shuttle', 'Business Center', 'Pet Friendly'];
const SECTIONS = ['Basic Info', 'Location', 'Amenities & Hours', 'Photos'];
const BASE = import.meta.env.VITE_API_URL?.replace('/api', '') || 'http://localhost:5000';

export default function EditHotel() {
  const { hotelId, hotels } = useOutletContext();
  const qc = useQueryClient();
  const coverRef = useRef();
  const galleryRef = useRef();

  const hotel = hotels?.find((h) => h.id === hotelId) || hotels?.[0];

  const [section, setSection] = useState(0);
  const [form, setForm] = useState({
    name: '', description: '', star_rating: '3', hotel_type: 'hotel',
    address: '', city: '', state: '', country: 'India',
    lat: '', lng: '', phone: '', website: '',
    check_in_time: '14:00', check_out_time: '11:00',
    amenities: [],
  });
  const [mapPos, setMapPos] = useState(null);
  const [coverFile, setCoverFile] = useState(null);
  const [coverPreview, setCoverPreview] = useState(null);
  const [galleryFiles, setGalleryFiles] = useState([]);

  useEffect(() => {
    if (!hotel) return;
    setForm({
      name: hotel.name || '',
      description: hotel.description || '',
      star_rating: String(hotel.star_rating || 3),
      hotel_type: hotel.hotel_type || 'hotel',
      address: hotel.address || '',
      city: hotel.city || '',
      state: hotel.state || '',
      country: hotel.country || 'India',
      lat: hotel.lat || '',
      lng: hotel.lng || '',
      phone: hotel.phone || '',
      website: hotel.website || '',
      check_in_time: hotel.check_in_time || '14:00',
      check_out_time: hotel.check_out_time || '11:00',
      amenities: hotel.amenities || [],
    });
    if (hotel.lat && hotel.lng) {
      setMapPos({ lat: parseFloat(hotel.lat), lng: parseFloat(hotel.lng) });
    }
  }, [hotel?.id]);

  const f = (field) => (e) => setForm((prev) => ({ ...prev, [field]: e.target.value }));

  const toggleAmenity = (a) => setForm((prev) => ({
    ...prev,
    amenities: prev.amenities.includes(a)
      ? prev.amenities.filter((x) => x !== a)
      : [...prev.amenities, a],
  }));

  const handleCover = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    if (file.size > 8 * 1024 * 1024) return toast.error('Image must be under 8MB');
    setCoverFile(file);
    setCoverPreview(URL.createObjectURL(file));
  };

  const handleGallery = (e) => {
    setGalleryFiles(Array.from(e.target.files).slice(0, 8));
  };

  const mutation = useMutation({
    mutationFn: (fd) => api.patch(`/hotels/${hotel.id}`, fd, {
      headers: { 'Content-Type': 'multipart/form-data' },
    }),
    onSuccess: () => {
      toast.success('Hotel updated successfully!');
      qc.invalidateQueries(['my-hotels']);
      qc.invalidateQueries(['hotel', hotel.id]);
      setCoverFile(null);
      setCoverPreview(null);
      setGalleryFiles([]);
    },
    onError: (err) => toast.error(err.response?.data?.message || 'Update failed'),
  });

  const handleSave = () => {
    if (!form.name.trim()) return toast.error('Hotel name is required');
    if (!form.address.trim()) return toast.error('Address is required');
    if (!form.city.trim()) return toast.error('City is required');

    const fd = new FormData();
    Object.entries(form).forEach(([k, v]) => {
      if (k === 'amenities') fd.append(k, JSON.stringify(v));
      else if (v !== '') fd.append(k, v);
    });
    if (mapPos) { fd.set('lat', mapPos.lat); fd.set('lng', mapPos.lng); }
    if (coverFile) fd.append('cover_image', coverFile);
    galleryFiles.forEach((img) => fd.append('images', img));

    mutation.mutate(fd);
  };

  if (!hotel) return (
    <div className="p-8 text-center text-white/40">No hotel found. Register one first.</div>
  );

  const currentCover = hotel.cover_image ? `${BASE}${hotel.cover_image}` : null;
  const currentGallery = hotel.images?.map((img) => `${BASE}${img}`) || [];

  return (
    <div className="p-4 md:p-8 max-w-3xl space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Edit Listing</h1>
          <p className="text-white/40 text-sm mt-1">{hotel.name}</p>
        </div>
        <button onClick={handleSave} disabled={mutation.isPending}
          className="bg-blue-500 hover:bg-blue-600 disabled:opacity-50 text-white font-semibold px-5 py-2.5 rounded-xl transition-all flex items-center gap-2">
          <Save size={15} /> {mutation.isPending ? 'Saving...' : 'Save Changes'}
        </button>
      </div>

      {/* Section tabs */}
      <div className="flex gap-1 glass rounded-xl p-1 overflow-x-auto">
        {SECTIONS.map((s, i) => (
          <button key={s} onClick={() => setSection(i)}
            className={`flex-1 py-2 px-3 text-xs font-medium rounded-lg whitespace-nowrap transition-all ${
              section === i ? 'bg-blue-500 text-white' : 'text-white/50 hover:text-white'
            }`}>
            {s}
          </button>
        ))}
      </div>

      {/* Section 0 — Basic Info */}
      {section === 0 && (
        <div className="card p-6 space-y-5 animate-fade-in">
          <h2 className="text-xs font-semibold text-white/40 uppercase tracking-wider flex items-center gap-2"><Info size={13} /> Basic Info</h2>

          <div>
            <label className="text-sm text-white/60 mb-1.5 block">Hotel Name *</label>
            <input value={form.name} onChange={f('name')} className="input" placeholder="The Grand Palace" />
          </div>

          <div>
            <label className="text-sm text-white/60 mb-1.5 block">Description</label>
            <textarea value={form.description} onChange={f('description')} className="input resize-none" rows={4}
              placeholder="What makes your hotel special..." />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-white/60 mb-1.5 block">Hotel Type</label>
              <select value={form.hotel_type} onChange={f('hotel_type')} className="input">
                {['hotel', 'resort', 'hostel', 'apartment', 'villa', 'boutique'].map((t) => (
                  <option key={t} value={t} className="capitalize">{t.charAt(0).toUpperCase() + t.slice(1)}</option>
                ))}
              </select>
            </div>
            <div>
              <label className="text-sm text-white/60 mb-1.5 block flex items-center gap-1"><Star size={12} /> Star Rating</label>
              <select value={form.star_rating} onChange={f('star_rating')} className="input">
                {[1, 2, 3, 4, 5].map((s) => (
                  <option key={s} value={s}>{'★'.repeat(s)}{'☆'.repeat(5 - s)} ({s} Star{s > 1 ? 's' : ''})</option>
                ))}
              </select>
            </div>
          </div>
        </div>
      )}

      {/* Section 1 — Location */}
      {section === 1 && (
        <div className="card p-6 space-y-5 animate-fade-in">
          <h2 className="text-xs font-semibold text-white/40 uppercase tracking-wider flex items-center gap-2"><MapPin size={13} /> Location</h2>

          <div>
            <label className="text-sm text-white/60 mb-1.5 block">Address *</label>
            <input value={form.address} onChange={f('address')} className="input" placeholder="123 Marine Drive" />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-white/60 mb-1.5 block">City *</label>
              <input value={form.city} onChange={f('city')} className="input" placeholder="Mumbai" />
            </div>
            <div>
              <label className="text-sm text-white/60 mb-1.5 block">State</label>
              <input value={form.state} onChange={f('state')} className="input" placeholder="Maharashtra" />
            </div>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-white/60 mb-1.5 block flex items-center gap-1"><Phone size={12} /> Phone</label>
              <input value={form.phone} onChange={f('phone')} className="input" placeholder="+91 22 1234 5678" />
            </div>
            <div>
              <label className="text-sm text-white/60 mb-1.5 block flex items-center gap-1"><Globe size={12} /> Website</label>
              <input value={form.website} onChange={f('website')} className="input" placeholder="https://..." />
            </div>
          </div>

          <div>
            <label className="text-sm text-white/60 mb-2 block">
              Pin Location <span className="text-white/30 font-normal">(click map to reposition)</span>
            </label>
            <div className="rounded-xl overflow-hidden border border-white/10">
              <MapPicker
                lat={mapPos?.lat || parseFloat(form.lat) || 19.076}
                lng={mapPos?.lng || parseFloat(form.lng) || 72.8777}
                onPick={(pos) => setMapPos(pos)}
                height="280px"
              />
            </div>
            {mapPos && (
              <p className="text-xs text-white/40 mt-1.5 flex items-center gap-1">
                <MapPin size={11} /> {mapPos.lat.toFixed(6)}, {mapPos.lng.toFixed(6)}
              </p>
            )}
          </div>
        </div>
      )}

      {/* Section 2 — Amenities & Hours */}
      {section === 2 && (
        <div className="card p-6 space-y-6 animate-fade-in">
          <h2 className="text-xs font-semibold text-white/40 uppercase tracking-wider flex items-center gap-2"><Clock size={13} /> Amenities & Hours</h2>

          <div>
            <label className="text-sm text-white/60 mb-3 block">Amenities</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {ALL_AMENITIES.map((a) => (
                <button key={a} type="button" onClick={() => toggleAmenity(a)}
                  className={`py-2 px-3 rounded-xl text-sm transition-all border text-left ${
                    form.amenities.includes(a)
                      ? 'bg-blue-500/20 border-blue-500/50 text-blue-300'
                      : 'border-white/10 text-white/50 hover:border-white/30 hover:text-white'
                  }`}>
                  {form.amenities.includes(a) ? '✓ ' : ''}{a}
                </button>
              ))}
            </div>
            {form.amenities.length > 0 && (
              <p className="text-xs text-blue-400 mt-2">{form.amenities.length} amenity selected</p>
            )}
          </div>

          <div>
            <label className="text-sm text-white/60 mb-3 block">Check-in / Check-out Times</label>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-white/40 mb-1 block">Check-in time</label>
                <input type="time" value={form.check_in_time} onChange={f('check_in_time')} className="input" />
              </div>
              <div>
                <label className="text-xs text-white/40 mb-1 block">Check-out time</label>
                <input type="time" value={form.check_out_time} onChange={f('check_out_time')} className="input" />
              </div>
            </div>
          </div>
        </div>
      )}

      {/* Section 3 — Photos */}
      {section === 3 && (
        <div className="card p-6 space-y-6 animate-fade-in">
          <h2 className="text-xs font-semibold text-white/40 uppercase tracking-wider flex items-center gap-2"><Image size={13} /> Photos</h2>

          {/* Cover */}
          <div>
            <label className="text-sm text-white/60 mb-2 block">Cover Image</label>
            <div className="w-full h-52 rounded-xl overflow-hidden bg-dark-700 flex items-center justify-center border-2 border-dashed border-white/10">
              {coverPreview || currentCover ? (
                <img src={coverPreview || currentCover} alt="cover" className="w-full h-full object-cover" />
              ) : (
                <div className="text-center">
                  <Upload size={24} className="text-white/30 mx-auto mb-2" />
                  <p className="text-sm text-white/40">No cover image</p>
                </div>
              )}
            </div>
            <div className="flex gap-2 mt-2">
              <button onClick={() => coverRef.current?.click()}
                className="btn-ghost text-xs flex items-center gap-1.5">
                <Upload size={12} /> {currentCover ? 'Replace Cover' : 'Upload Cover'}
              </button>
              {coverPreview && (
                <button onClick={() => { setCoverFile(null); setCoverPreview(null); }}
                  className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1 transition-colors">
                  <X size={12} /> Remove new
                </button>
              )}
            </div>
            <input ref={coverRef} type="file" accept="image/*" className="hidden" onChange={handleCover} />
          </div>

          {/* Gallery */}
          <div>
            <label className="text-sm text-white/60 mb-2 block">Gallery Images</label>

            {currentGallery.length > 0 && galleryFiles.length === 0 && (
              <div className="mb-3">
                <p className="text-xs text-white/40 mb-2">Current gallery ({currentGallery.length} photos)</p>
                <div className="flex gap-2 flex-wrap">
                  {currentGallery.map((src, i) => (
                    <img key={i} src={src} alt="" className="w-20 h-20 object-cover rounded-xl border border-white/10" />
                  ))}
                </div>
              </div>
            )}

            {galleryFiles.length > 0 && (
              <div className="mb-3">
                <p className="text-xs text-amber-400 mb-2">New gallery — will replace current ({galleryFiles.length} photos)</p>
                <div className="flex gap-2 flex-wrap">
                  {galleryFiles.map((file, i) => (
                    <img key={i} src={URL.createObjectURL(file)} alt="" className="w-20 h-20 object-cover rounded-xl border border-blue-500/40" />
                  ))}
                </div>
              </div>
            )}

            <div className="flex gap-2">
              <button onClick={() => galleryRef.current?.click()}
                className="btn-ghost text-xs flex items-center gap-1.5">
                <Upload size={12} /> {currentGallery.length ? 'Replace Gallery' : 'Upload Gallery'} (up to 8)
              </button>
              {galleryFiles.length > 0 && (
                <button onClick={() => setGalleryFiles([])}
                  className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1 transition-colors">
                  <X size={12} /> Clear
                </button>
              )}
            </div>
            <input ref={galleryRef} type="file" accept="image/*" multiple className="hidden" onChange={handleGallery} />
            <p className="text-xs text-white/30 mt-1">Uploading new gallery replaces all existing gallery images</p>
          </div>
        </div>
      )}

      {/* Bottom save */}
      <div className="flex items-center justify-between pt-2 pb-4">
        <p className="text-xs text-white/30">Changes are reflected on your public listing immediately</p>
        <button onClick={handleSave} disabled={mutation.isPending}
          className="bg-blue-500 hover:bg-blue-600 disabled:opacity-50 text-white font-semibold px-5 py-2.5 rounded-xl transition-all flex items-center gap-2">
          <Save size={15} /> {mutation.isPending ? 'Saving...' : 'Save Changes'}
        </button>
      </div>
    </div>
  );
}
