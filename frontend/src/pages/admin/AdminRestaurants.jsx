...
export default function AdminRestaurants() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-restaurants', search, status, page],
    queryFn: () => api.get('/admin/restaurants', { params: { search, status, page } }).then((r) => r.data),
  });

  const statusMutation = useMutation({
    mutationFn: ({ id, s }) => api.patch(`/admin/restaurants/${id}/status`, { status: s }),
    onSuccess: (_, vars) => { toast.success(`Restaurant ${vars.s}`); qc.invalidateQueries(['admin-restaurants']); qc.invalidateQueries(['admin-stats']); },
    onError: (err) => toast.error(err.response?.data?.message || 'Failed'),
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => api.delete(`/admin/restaurants/${id}`),
    onSuccess: () => { toast.success('Restaurant deleted'); qc.invalidateQueries(['admin-restaurants']); qc.invalidateQueries(['admin-stats']); setDeleteId(null); },
    onError: (err) => toast.error(err.response?.data?.message || 'Failed'),
  });
    ...
    <div className="p-4 md:p-8 space-y-6 animate-fade-in">
      ...
      {deleteId && (
        <div className="glass rounded-2xl p-5 border border-red-500/30 flex items-center gap-4">
          ...
          <p className="text-sm flex-1">Delete this restaurant? All its tables, bookings and reviews will be removed.</p>
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div className="text-center py-16 text-white/30">No restaurants found</div>
        ) : data?.restaurants?.map((r) => {
          const cfg = statusConfig[r.status] || statusConfig.active;
          const StatusIcon = cfg.icon;
          return (
            <div key={r.id} className="card p-4 md:p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <div className="flex items-center gap-4 w-full sm:w-auto sm:flex-1 min-w-0">
                {r.cover_image ? (
                  <img src={r.cover_image} alt={r.name} className="w-14 h-14 md:w-16 md:h-16 rounded-xl object-cover shrink-0" />
                ) : (
                  <div className="w-14 h-14 md:w-16 md:h-16 bg-brand-500/10 rounded-xl flex items-center justify-center text-2xl shrink-0">🍽️</div>
                )}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2 md:gap-3 text-xs text-white/40">
                    ...
                    <span className="flex items-center gap-1"><Star size={10} /> {parseFloat(r.avg_rating || 0).toFixed(1)} ({r.total_reviews})</span>
                    <span>{r.total_bookings} bookings</span>
                    <span className="text-white/20 hidden sm:inline">Owner: {r.owner_name}</span>
                  </div>
                  <p className="text-xs text-white/20 mt-1">Registered {format(new Date(r.created_at), 'MMM d, yyyy')}</p>
                </div>
              </div>

              <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                <Link to={`/restaurant/${r.id}`} target="_blank" className="w-8 h-8 glass rounded-lg flex items-center justify-center text-white/40 hover:text-white transition-colors" title="View page">
                  <ExternalLink size={13} />
                </Link>

                {r.status !== 'active' && (
                  <button onClick={() => statusMutation.mutate({ id: r.id, s: 'active' })}
                    className="text-xs bg-green-500/20 text-green-400 px-3 py-1.5 rounded-lg hover:bg-green-500/30 transition-colors">
                    Activate
                  </button>
                )}
                {r.status === 'active' && (
                  <button onClick={() => statusMutation.mutate({ id: r.id, s: 'suspended' })}
                ...
                <button onClick={() => setDeleteId(r.id)} className="w-8 h-8 glass rounded-lg flex items-center justify-center text-red-400/40 hover:text-red-400 transition-colors">
                  <Trash2 size={13} />
                </button>
              </div>
            </div>
